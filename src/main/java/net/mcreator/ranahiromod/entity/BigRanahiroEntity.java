...
public class BigRanahiroEntity extends Monster {
	public BigRanahiroEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(RanahiromodModEntities.BIG_RANAHIRO.get(), world);
	}

	public BigRanahiroEntity(EntityType<BigRanahiroEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(3, new NearestAttackableTargetGoal(this, LivingEntity.class, false, true) {
			...
			public boolean canUse() {
				double x = BigRanahiroEntity.this.getX();
				double y = BigRanahiroEntity.this.getY();
				double z = BigRanahiroEntity.this.getZ();
				Entity entity = BigRanahiroEntity.this;
				Level world = BigRanahiroEntity.this.level();
	...
	public static void init() {
		SpawnPlacements.register(RanahiromodModEntities.BIG_RANAHIRO.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
