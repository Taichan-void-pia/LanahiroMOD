...
import net.mcreator.ranahiromod.procedures.EpicDownSkeletonAIProcedure;
...
public class EpicDownSkeletonEntity extends Monster {
	public EpicDownSkeletonEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(RanahiromodModEntities.EPIC_DOWN_SKELETON.get(), world);
	}

	public EpicDownSkeletonEntity(EntityType<EpicDownSkeletonEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(3, new NearestAttackableTargetGoal(this, LivingEntity.class, false, true) {
			...
			public boolean canUse() {
				double x = EpicDownSkeletonEntity.this.getX();
				double y = EpicDownSkeletonEntity.this.getY();
				double z = EpicDownSkeletonEntity.this.getZ();
				Entity entity = EpicDownSkeletonEntity.this;
				Level world = EpicDownSkeletonEntity.this.level();
	...
	public void baseTick() {
		super.baseTick();
		EpicDownSkeletonAIProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	}

	public static void init() {
		SpawnPlacements.register(RanahiromodModEntities.EPIC_DOWN_SKELETON.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
			return RanahiroMODEntitySpawnConditionProcedure.execute(world, x, y, z);
		});
		DungeonHooks.addDungeonMob(RanahiromodModEntities.EPIC_DOWN_SKELETON.get(), 180);
	}
