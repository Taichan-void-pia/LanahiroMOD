...
import net.mcreator.ranahiromod.procedures.EpicDownZombieAIProcedure;
...
public class EpicDownZombieEntity extends Monster {
	public EpicDownZombieEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(RanahiromodModEntities.EPIC_DOWN_ZOMBIE.get(), world);
	}

	public EpicDownZombieEntity(EntityType<EpicDownZombieEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(3, new NearestAttackableTargetGoal(this, LivingEntity.class, false, true) {
			...
			public boolean canUse() {
				double x = EpicDownZombieEntity.this.getX();
				double y = EpicDownZombieEntity.this.getY();
				double z = EpicDownZombieEntity.this.getZ();
				Entity entity = EpicDownZombieEntity.this;
				Level world = EpicDownZombieEntity.this.level();
	...
	public void baseTick() {
		super.baseTick();
		EpicDownZombieAIProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	}

	public static void init() {
		SpawnPlacements.register(RanahiromodModEntities.EPIC_DOWN_ZOMBIE.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
			return RanahiroMODEntitySpawnConditionProcedure.execute(world, x, y, z);
		});
		DungeonHooks.addDungeonMob(RanahiromodModEntities.EPIC_DOWN_ZOMBIE.get(), 180);
	}
