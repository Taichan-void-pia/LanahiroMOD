...
import net.mcreator.ranahiromod.procedures.MinerSkeletonAIProcedure;
import net.mcreator.ranahiromod.init.RanahiromodModEntities;

import javax.annotation.Nullable;

public class MinerSkeletonEntity extends Monster {
	public MinerSkeletonEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(RanahiromodModEntities.MINER_SKELETON.get(), world);
	}

	public MinerSkeletonEntity(EntityType<MinerSkeletonEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(3, new NearestAttackableTargetGoal(this, LivingEntity.class, false, true) {
			...
			public boolean canUse() {
				double x = MinerSkeletonEntity.this.getX();
				double y = MinerSkeletonEntity.this.getY();
				double z = MinerSkeletonEntity.this.getZ();
				Entity entity = MinerSkeletonEntity.this;
				Level world = MinerSkeletonEntity.this.level();
	...
	public void baseTick() {
		super.baseTick();
		MinerSkeletonAIProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	}

	public static void init() {
		SpawnPlacements.register(RanahiromodModEntities.MINER_SKELETON.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
			return RanahiroMODEntitySpawnConditionProcedure.execute(world, x, y, z);
		});
		DungeonHooks.addDungeonMob(RanahiromodModEntities.MINER_SKELETON.get(), 180);
	}
