...
import net.mcreator.ranahiromod.procedures.DragonhiroKnightAIProcedure;
...
public class DragonhiroKnightEntity extends Monster {
	public DragonhiroKnightEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(RanahiromodModEntities.DRAGONHIRO_KNIGHT.get(), world);
	}

	public DragonhiroKnightEntity(EntityType<DragonhiroKnightEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(3, new NearestAttackableTargetGoal(this, LivingEntity.class, false, true) {
			...
			public boolean canUse() {
				double x = DragonhiroKnightEntity.this.getX();
				double y = DragonhiroKnightEntity.this.getY();
				double z = DragonhiroKnightEntity.this.getZ();
				Entity entity = DragonhiroKnightEntity.this;
				Level world = DragonhiroKnightEntity.this.level();
	...
	public void baseTick() {
		super.baseTick();
		DragonhiroKnightAIProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	}

	public static void init() {
		SpawnPlacements.register(RanahiromodModEntities.DRAGONHIRO_KNIGHT.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
