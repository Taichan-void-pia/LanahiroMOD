...
public class GeneralSkeletonEntity extends Monster {
	public GeneralSkeletonEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(RanahiromodModEntities.GENERAL_SKELETON.get(), world);
	}

	public GeneralSkeletonEntity(EntityType<GeneralSkeletonEntity> type, Level world) {
		super(type, world);
		setMaxUpStep(0.6f);
		xpReward = 0;
		setNoAi(false);
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(3, new NearestAttackableTargetGoal(this, LivingEntity.class, false, true) {
			...
			public boolean canUse() {
				double x = GeneralSkeletonEntity.this.getX();
				double y = GeneralSkeletonEntity.this.getY();
				double z = GeneralSkeletonEntity.this.getZ();
				Entity entity = GeneralSkeletonEntity.this;
				Level world = GeneralSkeletonEntity.this.level();
	...
	public static void init() {
		SpawnPlacements.register(RanahiromodModEntities.GENERAL_SKELETON.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
			return RanahiroMODEntitySpawnConditionProcedure.execute(world, x, y, z);
		});
		DungeonHooks.addDungeonMob(RanahiromodModEntities.GENERAL_SKELETON.get(), 180);
	}
