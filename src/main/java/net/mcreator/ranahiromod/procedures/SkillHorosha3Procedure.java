...
public class SkillHorosha3Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (entity.getPersistentData().getDouble("cnt1") < 20) {
			if (entity.getPersistentData().getDouble("cnt1") >= 8) {
				if (entity.getPersistentData().getDouble("cnt1") <= 18) {
					...
					if (world instanceof Level _level && !_level.isClientSide())
						_level.explode(null, (entity.getPersistentData().getDouble("x_pos")), (entity.getPersistentData().getDouble("y_pos") - 1), (entity.getPersistentData().getDouble("z_pos")), 0, Level.ExplosionInteraction.NONE);
					...
					for (int index3 = 0; index3 < 40; index3++) {
						world.addParticle((SimpleParticleType) (RanahiromodModParticleTypes.SK_LINER_BLADE_PARTICLE.get()), (xPos + Math.cos(Math.toRadians(Math.random() * 360)) * 4), yPos, (zPos + Math.sin(Math.toRadians(Math.random() * 360)) * 4),
								0, (0.8 + Math.random() * 0.4), 0);
					}
					if (world instanceof ServerLevel _level)
			...
			if (world instanceof ServerLevel _level)
				_level.sendParticles((SimpleParticleType) (RanahiromodModParticleTypes.SK_LINER_BLADE_PARTICLE.get()), (entity.getPersistentData().getDouble("x_pos")), (entity.getPersistentData().getDouble("y_pos") + 3),
						(entity.getPersistentData().getDouble("z_pos")), 30, 1, 1, 1, 0.5);
