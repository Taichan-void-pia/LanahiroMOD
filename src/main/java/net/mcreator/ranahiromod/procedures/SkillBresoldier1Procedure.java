...
public class SkillBresoldier1Procedure {
		...
		if (entity.getPersistentData().getDouble("cnt1") < 15) {
			...
			for (int index0 = 0; index0 < 2; index0++) {
				...
				for (int index1 = 0; index1 < 36; index1++) {
					...
					if (entity instanceof HypocriteEntity) {
						if (world instanceof ServerLevel _level)
							_level.getServer().getCommands().performPrefixedCommand(
									...
									"particle dust 0.3 0.3 0.3 1 ~ ~ ~ 0.05 0.05 0.05 1 1 force");
						if (world instanceof ServerLevel _level)
							_level.sendParticles(ParticleTypes.END_ROD, x_pos, y_pos, z_pos, 2, 0, 0, 0, 0.1);
					} else {
						if (world instanceof ServerLevel _level)
							_level.getServer().getCommands().performPrefixedCommand(
									new CommandSourceStack(CommandSource.NULL, new Vec3(x_pos, y_pos, z_pos), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
									"particle dust 0 1 0.5 1 ~ ~ ~ 0.05 0.05 0.05 1 1 force");
					}
