...
public class AIWardenhiroProcedure {
							...
							for (Entity entityiterator : _entfound) {
								if (entityiterator == (entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null)) {
									if (entityiterator instanceof Player) {
										if (!(entityiterator instanceof LivingEntity _livEnt23 && _livEnt23.hasEffect(MobEffects.INVISIBILITY))) {
											if (entityiterator instanceof LivingEntity _entity)
												_entity.setHealth(0);
										}
										if (entityiterator instanceof Player _player && !_player.level().isClientSide())
											_player.displayClientMessage(Component.literal(("\uFF0A" + entity.getDisplayName().getString() + "\u306F\u738B\u7834\u6563\u72B6\u3092\u5531\u3048\u305F\uFF01")), false);
