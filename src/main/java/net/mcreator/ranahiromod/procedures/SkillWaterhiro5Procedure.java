...
public class SkillWaterhiro5Procedure {
			...
			if (entity.getPersistentData().getDouble("cnt2") < 6) {
				...
				if (world instanceof ServerLevel _level)
					_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(xPos, yPos, zPos), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
							"particle dust 0.361 0.765 1.000 1 ~ ~ ~ 0.25 0.25 0.25 0.1 13 force");
				if (world instanceof ServerLevel _level)
					_level.sendParticles(ParticleTypes.BUBBLE, xPos, yPos, zPos, 5, 0.25, 0.25, 0.25, 0.2);
				if (world instanceof ServerLevel _level)
					_level.sendParticles(ParticleTypes.SPLASH, xPos, yPos, zPos, 5, 0.25, 0.25, 0.25, 0.2);
				for (int index0 = 0; index0 < 5; index0++) {
					...
					if (world instanceof ServerLevel _level)
						_level.sendParticles(ParticleTypes.CLOUD, xPos2, yPos2, zPos2, 10, 0.25, 0.25, 0.25, 0.2);
				...
				for (int index1 = 0; index1 < 20; index1++) {
					...
					for (int index2 = 0; index2 < 2; index2++) {
						...
						if (logic_a) {
							...
							xPos = entity.getPersistentData().getDouble("x_pos") + Math.cos(rad) * dis * 5;
							yPos = yPos2;
							zPos = entity.getPersistentData().getDouble("z_pos") + Math.sin(rad) * dis * 5;
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(xPos, yPos, zPos), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.player.splash.high_speed")), SoundSource.NEUTRAL, 1, 1);
					} else {
						_level.playLocalSound(xPos, yPos, zPos, ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("entity.player.splash.high_speed")), SoundSource.NEUTRAL, 1, 1, false);
					}
				}
				if (world instanceof Level _level && !_level.isClientSide())
