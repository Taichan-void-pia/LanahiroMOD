...
public class DragonhiroKnightAIProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity.isAlive()) {
			if (entity.getPersistentData().getDouble("cnt_y") == 0) {
				...
				if (rnd <= 0.5) {
					if (entity instanceof LivingEntity _entity) {
						ItemStack _setstack = new ItemStack(RanahiromodModItems.DRAGON_SWORD.get()).copy();
					...
					if (entity instanceof LivingEntity _entity) {
						ItemStack _setstack = new ItemStack(RanahiromodModItems.DRAGON_HALBERD.get()).copy();
						_setstack.setCount(1);
						_entity.setItemInHand(InteractionHand.MAIN_HAND, _setstack);
						if (_entity instanceof Player _player)
							_player.getInventory().setChanged();
			...
			if (entity.getPersistentData().getDouble("mode") == 1) {
				SkillDragonhiroKnight1Procedure.execute(world, x, y, z, entity);
