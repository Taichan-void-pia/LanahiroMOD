...
public class AllTargetDamage3Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		}
		{
			...
			for (Entity entityiterator : _entfound) {
				guard = false;
				just_guard = false;
				logic_attack = !(entityiterator == entity);
				logic_attack2 = logic_attack && entityiterator instanceof LivingEntity;
				if (world.getLevelData().getGameRules().getBoolean(RanahiromodModGameRules.LANAHIROMODPVP)) {
					logic_attack2 = logic_attack2 || logic_attack && entityiterator instanceof Player;
				}
				if (logic_attack2) {
					if (mainhandweapon.getItem() == Items.BOW || mainhandweapon.getItem() == Items.CROSSBOW) {
						...
						if (EnchantmentHelper.getItemEnchantmentLevel(Enchantments.BANE_OF_ARTHROPODS, mainhandweapon) != 0 && entityiterator instanceof LivingEntity _livEnt30 && _livEnt30.getMobType() == MobType.ARTHROPOD) {
					...
					if (entity instanceof LivingEntity _livEnt33 && _livEnt33.hasEffect(MobEffects.DAMAGE_BOOST)) {
					...
					if (mainhandweapon.getItem() == Items.BOW || mainhandweapon.getItem() == Items.CROSSBOW) {
						...
						if (EnchantmentHelper.getItemEnchantmentLevel(Enchantments.SHARPNESS, mainhandweapon) != 0) {
							if (!(entity instanceof LivingEntity _livEnt40 && _livEnt40.hasEffect(MobEffects.DIG_SLOWDOWN))) {
								damage_sorce = damage_sorce + 3 * (1 + strength_level / 20) * mainhandweapon.getEnchantmentLevel(Enchantments.SHARPNESS);
							}
						}
						if (EnchantmentHelper.getItemEnchantmentLevel(Enchantments.SMITE, mainhandweapon) != 0 && entityiterator instanceof LivingEntity _livEnt43 && _livEnt43.getMobType() == MobType.UNDEAD) {
						...
						if (EnchantmentHelper.getItemEnchantmentLevel(Enchantments.BANE_OF_ARTHROPODS, mainhandweapon) != 0 && entityiterator instanceof LivingEntity _livEnt47 && _livEnt47.getMobType() == MobType.ARTHROPOD) {
					...
					if (entity instanceof LivingEntity _livEnt70 && _livEnt70.hasEffect(MobEffects.DIG_SLOWDOWN)) {
					...
					if (!noDamaged) {
						guard = false;
						just_guard = false;
						if (entityiterator instanceof LivingEntity _livEnt74 && _livEnt74.hasEffect(RanahiromodModMobEffects.GUARD_PROTECTION.get())) {
