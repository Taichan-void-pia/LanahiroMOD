...
public class CraftingTableCanBreakProcedure {
	@SubscribeEvent
		...
		if (new Object() {
			...
			if ((world.getBlockState(BlockPos.containing(x, y, z))).getBlock() == Blocks.CRAFTING_TABLE) {
				if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getItem() instanceof AxeItem) {
					if ((entity instanceof LivingEntity _livEnt ? _livEnt.getMainHandItem() : ItemStack.EMPTY).getMaxDamage() <= 800) {
						logic_cancel = true;
					}
				} else {
					logic_cancel = true;
