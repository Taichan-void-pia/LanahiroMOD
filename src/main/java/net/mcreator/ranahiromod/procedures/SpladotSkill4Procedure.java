...
public class SpladotSkill4Procedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (entity.getPersistentData().getDouble("cnt1") < 10) {
			...
			for (int index1 = 0; index1 < 2; index1++) {
				for (int index2 = 0; index2 < (int) (entity.getPersistentData().getDouble("cnt1") * 8); index2++) {
					...
					if (world instanceof ServerLevel _level)
						_level.sendParticles(ParticleTypes.CLOUD, xPos, yPos, zPos, 2, 0, 0, 0, 0);
			...
			if (world instanceof ServerLevel _level)
				_level.sendParticles(ParticleTypes.SOUL_FIRE_FLAME, xPos, yPos, zPos, 40, 3, 0, 3, 1);
			for (int index3 = 0; index3 < 50; index3++) {
				world.addParticle(ParticleTypes.FLAME, (xPos + Math.cos(Math.toRadians(Math.random() * 360)) * 4), yPos, (zPos + Math.sin(Math.toRadians(Math.random() * 360)) * 4), 0, (0.8 + Math.random() * 0.4), 0);
			}
