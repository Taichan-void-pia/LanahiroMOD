...
public class SkillLavahiro3Procedure {
		...
		if (entity.getPersistentData().getDouble("cnt1") < 20) {
			...
			for (int index0 = 0; index0 < 11; index0++) {
				...
				if (world instanceof ServerLevel _level)
					_level.sendParticles(ParticleTypes.FLAME, xPos, yPos, zPos, 15, 0.25, 0.25, 0.25, 0.05);
				if (world instanceof ServerLevel _level)
					_level.sendParticles(ParticleTypes.LAVA, xPos, yPos, zPos, 5, 0.25, 0.25, 0.25, 0);
				if (world instanceof ServerLevel _level)
					_level.sendParticles(ParticleTypes.FIREWORK, xPos, yPos, zPos, 2, 0.25, 0.25, 0.25, 0.1);
				if (world instanceof ServerLevel _level)
					_level.getServer().getCommands().performPrefixedCommand(new CommandSourceStack(CommandSource.NULL, new Vec3(xPos, yPos, zPos), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
							"particle dust 1.000 0.200 0.227 2 ~ ~ ~ 0.25 0.25 0.25 0.1 10 force");
