...
public class SkillDragonhiroKnightSummonProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		for (int index0 = 0; index0 < (int) (1 + Math.round(Math.random() * 0)); index0++) {
			...
			if (world instanceof ServerLevel _level) {
				Entity entityToSpawn = RanahiromodModEntities.DRAGONHIRO_KNIGHT.get().spawn(_level, BlockPos.containing(xPos, yPos, zPos), MobSpawnType.MOB_SUMMONED);
