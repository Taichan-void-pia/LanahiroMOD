...
public class SkillVazafila1Procedure {
			...
			for (int index0 = 0; index0 < 2; index0++) {
				...
				for (int index1 = 0; index1 < 36; index1++) {
					...
					for (int index2 = 0; index2 < 4; index2++) {
						...
						if (entity instanceof LivingEntity _livEnt30 && _livEnt30.hasEffect(RanahiromodModMobEffects.LOW_ATTACK_REINFORCED.get())) {
							if (Math.random() < 0.5) {
							...
							if (Math.random() < 0.5) {
								if (world instanceof ServerLevel _level)
									_level.getServer().getCommands().performPrefixedCommand(
											new CommandSourceStack(CommandSource.NULL, new Vec3(xPos, yPos, zPos), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
											"particle dust 0.627 0.141 1.000 1 ~ ~ ~ 0 0 0 1 0 force");
							} else {
								if (world instanceof ServerLevel _level)
									_level.getServer().getCommands().performPrefixedCommand(
											new CommandSourceStack(CommandSource.NULL, new Vec3(xPos, yPos, zPos), Vec2.ZERO, _level, 4, "", Component.literal(""), _level.getServer(), null).withSuppressedOutput(),
											"particle dust 0.271 0.063 0.431 1 ~ ~ ~ 0 0 0 1 0 force");
							}
						}
